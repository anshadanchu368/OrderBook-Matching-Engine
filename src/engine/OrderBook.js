...
import {
  createBookUpdatedEvent,
  createOrderAcceptedEvent,
  createOrderCancelledEvent,
  createOrderFillEvent,
  createOrderRestedEvent,
  createTradeCreatedEvents,
  createStopOrderAcceptedEvent,
  createStopOrderTriggeredEvent,
} from "./DomainEvent.js";
...
export class OrderBook {
  ...
  placeLimitOrder({
    ...
    const events = [
      createOrderAcceptedEvent(incomingOrder.snapshot()),
      ...createTradeCreatedEvents(trades),
      createOrderFillEvent(incomingOrder.snapshot())
    ]
    ...
    if (incomingOrder.isActive && incomingOrder.remainingQuantity > 0) {
      ...
      events.push(createOrderRestedEvent(incomingOrder.snapshot()))
    }
  ...
  placeTrailingStopMarketOrder({
    ...
    const orderSnapshot = this.getStopOrderSnapshot(trailingStopOrder);
  ...
  placeMarketOrder({
    ...
    placeStopMarketOrder({
      ...
      const orderSnapshot = this.getStopOrderSnapshot(stopOrder);
 ...
 processStopOrdersAfterTrades(initialTrades) {
  ...
  while (latestTrades.length > 0) {
    ...
    for (const stopOrder of eligibleStopOrders) {
      ...
      const stopOrderSnapshot = this.getStopOrderSnapshot(stopOrder);

      events.push(createStopOrderTriggeredEvent(stopOrderSnapshot));

      const triggeredResult = this.executeTriggeredStopOrder(stopOrder);

      triggeredOrders.push(stopOrderSnapshot);
      latestTrades.push(...triggeredResult.trades);
      allTriggeredTrades.push(...triggeredResult.trades);

      events.push(...createTradeCreatedEvents(triggeredResult.trades));

      if (triggeredResult.order) {
        events.push(createOrderFillEvent(triggeredResult.order));
      }

      if (
        triggeredResult.order &&
        triggeredResult.order.status === OrderStatus.OPEN &&
        triggeredResult.order.remainingQuantity > 0
      ) {
        events.push(createOrderRestedEvent(triggeredResult.order));
      }
    }
  }
